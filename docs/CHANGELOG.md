...
## [2026-01-19 14:00] 安全与体验双重升级

**Action**: 增强系统安全性和用户体验细节

**Key Updates**:

1.  **Security Hardening (安全加固)**:
    - 🔒 **Rate Limiting**: 引入了 `@nestjs/throttler` 全局限流中间件。
    - **Policy**: 限制单 IP 每分钟最多请求 **60次**，有效防止 API 被恶意刷爆或 DDo S。
    - **Guard**: 增加了话题护栏 (Topic Guard)，在 System Prompt 层面严禁 AI 回答非旅游相关问题（如写代码、聊八卦），确保专在专用。

2.  **Context Enhancement (智能记忆)**:
    - 🧠 **Context Awareness**: 后端 LangChain 服务现在会扫描完整的对话历史来锁定“目的地城市”。
    - **Outcome**: 用户说“太贵了”时，AI 不需要用户再说一遍城市名，就能自动结合之前的城市上下文重新规划。
...
## [2026-01-20 08:30] AI 上下文优化 - 智能出发地推断
...
## [2026-01-20 09:45] 行程展示体验优化 (UI Polish)

- **Fix (Weather)**: 增加了天气服务的超时时间 (3s -> 8s)，解决了部分城市天气加载失败的问题。
- **Fix (Parser)**: 修复了“实用小贴士”解析逻辑，防止“预算明细”等后续段落被错误包含在 Tips 框中。
- **UI (Index)**: 修正了行程天数序号显示问题 (从 1 开始计数)。
- **UI (Price)**: 优化了价格显示逻辑，金额为 0 或 0元 时直接显示更加友好的“免费”标签。

## [2026-01-20 10:15] 用户体验优化 (UX Improvements)
